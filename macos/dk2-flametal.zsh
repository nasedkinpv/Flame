...
readonly PREFIX="${HOME}/Library/Application Support/Dungeon Keeper II/prefix-native"
...
show_error() {
  /usr/bin/osascript -e "display alert \"Dungeon Keeper II\" message \"$1\" as critical" >/dev/null
}
...
if [[ ! -f "${GAME_DIR}/DKII-DX.exe" || ! -f "${GAME_DIR}/flametal/Flametal.dll" ]]; then
...
"${RUNTIME}/bin/wine" 'C:\GOG Games\Dungeon Keeper 2\DKII-DX.exe' \
  -skip-launcher \
  -flametal:windowed \
  -flametal:no-initial-size \
  -flametal:game-res=1024x768 \
  -nomovies \
  -disablegamma \
  >>"${LOG_FILE}" 2>&1
