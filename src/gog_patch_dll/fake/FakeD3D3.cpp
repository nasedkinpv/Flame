...
HRESULT FakeD3D3::FindDevice(LPD3DFINDDEVICESEARCH, LPD3DFINDDEVICERESULT) {
    gog_unused_function_called("FakeD3D3::FindDevice");
    return DDERR_GENERIC;
}
...
HRESULT FakeD3D3::CreateDevice(REFCLSID riid, LPDIRECTDRAWSURFACE4 surf_, LPDIRECT3DDEVICE3 *a4, LPUNKNOWN a5) {
    ...
    if (!IsEqualGUID(IID_IDirect3DHALDevice, riid)) gog_assert_failed("FakeD3D3::CreateDevice:1011");
    ...
    if (metal_bridge::isEnabled()) {
        ...
        if (FAILED(hr)) {
            gog_assert_failed_hr("FakeD3D3::CreateDevice:1021", hr);
        }
    }
    *a4 = new FakeDevice3();
    return DD_OK;
...
HRESULT FakeD3D3::CreateVertexBuffer(LPD3DVERTEXBUFFERDESC, LPDIRECT3DVERTEXBUFFER *, DWORD, LPUNKNOWN) {
    gog_unused_function_called("FakeD3D3::CreateVertexBuffer");
    return DDERR_GENERIC;
}

HRESULT FakeD3D3::EnumZBufferFormats(REFCLSID, LPD3DENUMPIXELFORMATSCALLBACK, LPVOID) {
    gog_unused_function_called("FakeD3D3::EnumZBufferFormats");
    return DDERR_GENERIC;
}

HRESULT FakeD3D3::EvictManagedTextures(void) {
    gog_unused_function_called("FakeD3D3::EvictManagedTextures");
    return DDERR_GENERIC;
}
